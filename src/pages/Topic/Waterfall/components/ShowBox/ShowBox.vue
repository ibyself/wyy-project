<template>
    <div>
        <div class="show_box" v-for="(topic,index) in topics1" :key="index">
            <div class="short_box" v-if="topic.style===1 || topic.style===0">
                <div class="imgWrap">
                    <img v-if="topic.newAppBanner" :src="topic.newAppBanner" alt="">
                    <img v-else :src="topic.picUrl" alt="">
                </div>
                <div class="lookDesc">{{topic.title}}</div>
                <div class="lookDetail">
                    <div class="userInfo">
                        <img :src="topic.avatar"  alt="">
                        <div class="userName">{{topic.nickname}}</div>
                    </div>
                    <div class="amount">
                        <img src="data:image/png;base64," alt="">
                        <div class="count">{{(topic.readCount+'').slice(0,-3)+'k'}}</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="show_box" v-for="(topic,index) in topics2" :key="index">
            <div class="long_box" v-if="topic.style===2">
                <div class="imgWrap">
                    <img v-if="topic.newAppBanner" :src="topic.newAppBanner" alt="">
                    <img v-else :src="topic.picUrl" alt="">
                </div>
                <div class="lookDesc">{{topic.title}}</div>
                <div class="lookDetail">
                    <div class="userInfo">
                        <img :src="topic.avatar"  alt="">
                        <div class="userName">{{topic.nickname}}</div>
                    </div>
                    <div class="amount">
                        <img src="data:image/png;base64," alt="">
                        <div class="count">{{topic.readCount}}</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script  type="text/ecmascript-6">
    export default {
        props:{
            topics1:{
                type:Array
            },
            topics2:{
                type:Array
            }
        }
    };
</script>

<style lang='stylus' rel='stylesheet/stylus' scoped>
    .show_box
        .short_box,.long_box
            .imgWrap
                width 342px
                border-top-left-radius .24rem
                border-top-right-radius .24rem
                overflow hidden
                img 
                    width 100%
                    height 100%

            .lookDesc
                font-size 28px
                background-color #fff
                padding 18px 16px 0
                color #333
                overflow hidden
                text-overflow ellipsis
            .lookDetail
                padding 16px 16px 24px
                background-color #fff
                font-size 24px
                display flex
                justify-content space-between
                height 48px
                border-bottom-left-radius .24rem
                border-bottom-right-radius .24rem
                color #7f7f7f
                .userInfo
                    display flex
                    align-items center
                    img
                        width 48px
                        height 48px
                        border-radius 24px
                        
                    .userName
                        margin-left 8px
                .amount
                    display flex
                    align-items center
                    img
                        width 48px
                        height 48px
                        border-radius 24px
    
</style>
